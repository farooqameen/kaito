...
class Inference(CustomLLM):
    ...
    async def acomplete(
        ...
        try:
            if LLM_INFERENCE_URL.startswith(OPENAI_URL_PREFIX):
                return await self._async_openai_complete(
                    prompt, **kwargs, **self.params
                )
            elif LLM_INFERENCE_URL.startswith(HUGGINGFACE_URL_PREFIX):
                return await self._async_huggingface_remote_complete(
                    prompt, **kwargs, **self.params
                )
            else:
                return await self._async_custom_api_complete(
                    prompt, **kwargs, **self.params
                )
        except HTTPException as http_exc:
            raise http_exc
    ...
    async def _async_openai_complete(
        self, prompt: str, **kwargs: Any
    ) -> CompletionResponse:
        return await OpenAI(api_key=LLM_ACCESS_SECRET, **kwargs).acomplete(prompt)

    async def _async_huggingface_remote_complete(
        self, prompt: str, **kwargs: Any
    ) -> CompletionResponse:
    ...
    async def _async_custom_api_complete(
        self, prompt: str, **kwargs: Any
    ) -> CompletionResponse:
